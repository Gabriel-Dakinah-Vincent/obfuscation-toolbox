...
def deobfuscate(method: str, code: str, **params) -> str:
    if not code:
        return code
    if method not in OBFUSCATOR_REGISTRY:
        available = ', '.join(OBFUSCATOR_REGISTRY.keys())
        raise ValueError(f"Unknown deobfuscation method: {method}. Available: {available}")
    
    obfuscator_class = OBFUSCATOR_REGISTRY[method]
    obfuscator = obfuscator_class(**params)
    return obfuscator.deobfuscate(code)
